...
5. **Ảnh kết quả**: vẽ **vòng XANH** ở đáp án đúng cho mọi câu; **vòng ĐỎ** ở
   mọi ô thí sinh đã tô/detect được (câu đúng có cả 2 vòng); vòng xanh dương ở
   chữ số MSSV nhận diện được.
...
POST /api/omr/process   (multipart/form-data)
  ...
  identity_only    (tùy chọn) "1" để chỉ căn trang + đọc MSSV, bỏ qua MCQ
...
## Lưu ý khi chụp bằng mobile auto-capture

- Giữ điện thoại đứng yên khoảng 1-2 giây sau khi 4 điểm neo đã nằm đúng trong
  overlay. Frontend sẽ đợi neo ổn định và chọn khung nét nhất gần thời điểm chụp,
  nên không cần lắc/di chuyển liên tục để "kích" auto-capture.
- Để trang lấp đầy khung A4 nhưng không cắt mất 4 ô vuông đen. Nếu chỉ thấy
  một phần góc hoặc ảnh quá lệch, auto-capture sẽ chờ thay vì chụp ngay.
- Với trang đầu, ưu tiên đủ sáng và nét hơn tốc độ. Nếu UI không tự chụp, bật
  flash/đổi nguồn sáng, đặt phiếu lên nền tối, rồi giữ yên lại.
- Sau khi trang đầu tự chụp, hệ thống đọc MSSV ở nền. Nếu cảnh báo `aligned < 4`
  hoặc chưa nhận diện MSSV, hãy chụp lại trang đầu hoặc gán thí sinh thủ công.
